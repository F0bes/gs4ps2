...
static void ethdprintIPConfig(void)
{
	...
	{
		dprint("Unable to read IP address.\n");
	}
}
...
s32 network_init(void)
{
	...
	s32 dhcp = *(s32*)CFG_VALS[CFG_OPT_DHCP];
	if (!dhcp)
	{
		IP = (struct ip4_addr*)CFG_VALS[CFG_OPT_IP];
		NM = (struct ip4_addr*)CFG_VALS[CFG_OPT_NM];
		GW = (struct ip4_addr*)CFG_VALS[CFG_OPT_GW];

		if (!IP)
		{
			dprint("IP address not set in configuration, and DHCP is disabled\n");
			return -1;
		}
		if (!NM)
		{
			dprint("Netmask not set in configuration, and DHCP is disabled\n");
			return -1;
		}
		if (!GW)
		{
			dprint("Gateway not set in configuration, and DHCP is disabled\n");
			return -1;
		}
	}
	else
	...
	{
		dprint("Error: failed to set link mode.\n");
		goto failure;
	}
	...
	ethApplyIPConfig(dhcp, IP, NM, GW);
	// Wait for the link to become ready.
	dprint("Waiting for connection...\n");
	if (ethWaitValidNetIFLinkState() != 0)
	{
		dprint("Error: failed to get valid link status.\n");
		goto failure;
	}

	if (dhcp)
	{
		dprint("Waiting for DHCP lease...");
